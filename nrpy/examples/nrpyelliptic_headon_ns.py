...
project_name = "nrpyelliptic_headon_ns"
fp_type = "double"
...
commonClib.register_CFunction_variable_wavespeed_gfs_all_points(CoordSystem=CoordSystem)

# Generate functions to set AUXEVOL gridfunctions
nrpyellClib.register_CFunction_compute_single_source_term(SourceType=SourceType)
nrpyellClib.register_CFunction_auxevol_gfs_all_points(
    SourceType=SourceType, OMP_collapse=OMP_collapse
)

# Generate function that calls functions to set variable wavespeed and all other AUXEVOL gridfunctions
commonClib.register_CFunction_initialize_constant_auxevol()
...
commonClib.register_CFunction_compute_L2_norm_of_gridfunction(CoordSystem=CoordSystem)
...
if (strncmp(commondata->outer_bc_type, "radiation", 50) == 0) {
  ...
  const REAL custom_gridfunctions_wavespeed[4] = {wavespeed_at_outer_boundary, wavespeed_at_outer_boundary,
                                                  wavespeed_at_outer_boundary, wavespeed_at_outer_boundary};
