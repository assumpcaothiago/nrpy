"""
Construct C function to compute Tmunu.
...
def register_CFunction_compute_rescaledvU_from_vCartU(
    ...
    params = "const commondata_struct *restrict commondata, const params_struct *restrict params, REAL vCartU[3]"
    params += "REAL *restrict rescaledvU0, REAL *restrict rescaledvU1, REAL *restrict rescaledvU2"

    # Compute reference-metric quantities
    rfm = refmetric.reference_metric[CoordSystem + "_rfm_precompute" if enable_rfm_precompute else CoordSystem]
    ...
    vU = jac.basis_transform_vectorU_from_Cartesian_to_rfmbasis(CoordSystem=CoordSystem, Cart_src_vectorU=vCartU)
...
def register_CFunction_compute_T4UU(
    ...
    const REAL rescaledvU0, const REAL rescaledvU1, const REAL rescaledvU2, REAL *restrict in_gfs, const REAL *restrict auxevol_gfs"""
    ...
    for mu in range(4):
        for nu in range(4):
            ...
            varnames.append(gri.BHaHGridFunction.access_gf(f"T4UU{mu}{nu}", gf_array_name="auxevol_gfs"))
...
if __name__ == "__main__":
    ...
    register_CFunction_compute_rescaledvU_from_vCartU(
        CoordSystem=CoordSystem,
        enable_rfm_precompute=False,
    )

    register_CFunction_compute_u4Ut(
        CoordSystem=CoordSystem,
        enable_rfm_precompute=False,
        enable_GoldenKernels=False,
    )

    register_CFunction_compute_T4UU(CoordSystem=CoordSystem)
    # Print function
    print(cfc.CFunction_dict[f"compute_rescaledvU_from_vCartU__rfm__{CoordSystem}"].full_function)
    print(cfc.CFunction_dict[f"compute_u4Ut__rfm__{CoordSystem}"].full_function)
    print(cfc.CFunction_dict["compute_T4UU"].full_function)