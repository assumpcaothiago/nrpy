...
4. One evolution grid function array (uu) of size NRPYELL_TOTAL_PTS
"""
    cfunc_type = "void"
    ...
    REAL *restrict xx[3];
    for (int ww = 0; ww < 3; ww++) xx[ww] = griddata[0].xx[ww];
    ...
    const REAL *NRPYELL_uu = &y_n_gfs[IDX4(UUGF, 0, 0, 0)];
    ...
    if (fwrite(NRPYELL_uu, sizeof(REAL), NRPYELL_TOTAL_PTS, fp) != (size_t)NRPYELL_TOTAL_PTS) {
        perror("Error writing NRPYELL_uu");
        fclose(fp);
        exit(1);
    }
...
def register_CFunction_read_NRPYELL_binary() -> Union[None, pcg.NRPyEnv_type]:
    """
    Register the C function that reads minimal NRPyElliptic data from binary file.
...
2. Six REALs:
   - NRPYELL_AMAX, NRPYELL_bScale, NRPYELL_SINHWAA,
     NRPYELL_dxx0, NRPYELL_dxx1, NRPYELL_dxx2
3. Three coordinate arrays (of sizes NRPYELL_Nxx_plus_2NGHOSTS0,
   NRPYELL_Nxx_plus_2NGHOSTS1, and NRPYELL_Nxx_plus_2NGHOSTS2)
4. One evolution grid function array (uu) of size NRPYELL_TOTAL_PTS
...
@param NRPYELL_uu                Pointer to a REAL* that will be allocated for the evolution array uu.
"""
    cfunc_type = "void"
...
"""
  ...
  *NRPYELL_xx2 = malloc((*NRPYELL_Nxx_plus_2NGHOSTS2) * sizeof(REAL));
  ...
  // Allocate and read evolution grid function arrays
  *NRPYELL_uu = malloc((*NRPYELL_TOTAL_PTS) * sizeof(REAL));
  if (*NRPYELL_uu == NULL) {
    perror("Memory allocation failed for NRPYELL_uu");
    fclose(fp);
    exit(1);
  }
  if (fread(*NRPYELL_uu, sizeof(REAL), *NRPYELL_TOTAL_PTS, fp) != (size_t)*NRPYELL_TOTAL_PTS) {
    perror("Error reading evolution array NRPYELL_uu");
    fclose(fp);
    exit(1);
  }
