...
diagnostic_gfs; optionally exports raytracing data; populates diagnostic_gfs via
diagnostic_gfs_set(...); conditionally calls diagnostics that consume
diagnostic_gfs; releases temporary storage; and advances the progress indicator.
Hooks to free and later restore MoL scratch arrays are present but currently
disabled in the emitted code.
...
def _register_CFunction_diagnostics(  # pylint: disable=unused-argument
    ...
    and on output steps it allocates diagnostic_gfs, optionally exports raytracing data,
    populates diagnostic_gfs via
    diagnostic_gfs_set(...), invokes diagnostics that consume diagnostic_gfs, then frees
    temporary storage.
 ...
 * steps it allocates per-grid temporary arrays (diagnostic_gfs), optionally exports
 * raytracing data, initializes
 * diagnostic_gfs via diagnostic_gfs_set(...), and then runs diagnostics that consume
 * diagnostic_gfs, such as diagnostics_nearest(...), diagnostics_interp(...), and
...
def register_all_diagnostics(
    ...
    - Registers helper C functions for:
        - nearest diagnostics (several sampling helpers),
        - volume integration (volume-element helper), and
        - optional raytracing binary exporters.
    ...
    :raises ValueError: If raytracing-data export is enabled for more than one
        ...
        also currently support only one runtime grid; generated C aborts unless
